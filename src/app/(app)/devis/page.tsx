            ...
            {rows.map(({ quote: q, customerName }) => {
              ...
              return (
                <tr key={q.id} className="border-t" style={{ borderColor: "var(--border)" }}>
                  <td className="p-3 font-mono"><Link className="underline" href={`/devis/${q.id}`}>{q.number ?? "Brouillon"}</Link></td>
                  ...
                  <td className="p-3">{QUOTE_LABELS[q.status]}{expired && <span className="ml-2 text-xs" style={{ color: "var(--danger)" }}>expiré</span>}</td>
                </tr>
              );
            })}
