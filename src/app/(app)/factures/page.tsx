...
export default async function InvoicesPage({
  ...
  searchParams: Promise<{ q?: string; kind?: string; status?: string; page?: string; ok?: string; error?: string }>;
}) {
  ...
  const { rows, total, pageSize } = await listInvoices(db, { q: sp.q, kind, status, page });
  const qs = (p: number) =>
    `/factures?${new URLSearchParams({
      ...(sp.q ? { q: sp.q } : {}), ...(kind ? { kind } : {}), ...(status ? { status } : {}), page: String(p),
    })}`;
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left" style={{ color: "var(--muted)" }}>
              ...
              <th className="p-3 text-right">Total TTC</th><th className="p-3 text-right">Net à payer</th><th className="p-3">Statut</th>
            </tr>
          </thead>
          <tbody>
            {rows.map(({ invoice: i, customerName }) => (
            ...
            {rows.length === 0 && <tr><td className="p-3" colSpan={8} style={{ color: "var(--muted)" }}>Aucun document.</td></tr>}
          </tbody>
        </table>
      </div>
